...
class Amino:
    ...
    def set_modification_params(self, modif_params):
        ...
        self.natural_modification = modif_params["natural_modification"]
...
class Sequence:
    def __init__(self, accession, sequence, proteins_dic, modifications_dic):
    ...
    def modification_params(self, number):
        str_number = str(float(number))
        # print("mass: " + str(number) + " -> " + str(float(number)))
        if str_number in self.modifications_dic:
            return self.modifications_dic[str_number]
        else:
            return {
                "mod1": "unknown",
                "mod2": "",
        ...
        for char in self.sequence_string:
            if not modif_number_mode:
                ...
                if char != "]":
                    ...
                    modif_number_mode = False
                    sequence_array[len(sequence_array) - 1].set_modification_params(self.modification_params(modif_number))
